...
`;
  ...
  if (ctx2.root) {
    result_css += create_root_default(ctx2);
  }
  if (ctx2.wrapper) {
...
function prerender_default(ctx2) {
  if (!ctx2.dark) {
  ...
  NODES.forEach((node) => {
    for (const attr of ATTRS) {
      ...
      for (const token of ATTR_VALUE.trim().split(/\s+/g)) {
        if (!updateStore(ctx2, token, attr)) {
          continue;
        }
...
var StatesParser = class {
  constructor(ctx2) {
    this.ctx = ctx2;
    this.parseMedia = new MediaParser(ctx2);
  }
  parse(state, attr) {
...
var RuleParser = class {
  ...
  parse(path2, object) {
    ...
    const PARTS = path2.replace(RE_PATH, (_, esc) => esc ? SPLIT_SYMBOL : "\n").split("\n");
    let array_path = [];
...
function create_var_default(ctx2, variable, opacity = "") {
  if (is.var(variable)) {
  ...
  let result = var_color_default(ctx2, variable);
  if (result) {
...
function create_color_default(ctx2, color, opacity) {
  if (color[0] == "$") {
    return create_var_default(ctx2, color, opacity);
  }
  if (color[0] === "#") {
    if (color.length === 4) {
      let r = color[1];
...
var StylesParser = class {
  constructor(ctx2) {
    this.ctx = ctx2;
    this.parseValue = new ValueParser(ctx2);
    this.parseRule = new RuleParser(ctx2);
  }
  parse(style, attr, token, states) {
    ...
    let { source, path: path2, value } = this.parseRule.parse(style, target);
    const params = {
      token,
      style,
      path: path2,
...
var Parser = class {
  constructor(ctx2) {
    this.ctx = ctx2;
    this.parseStates = new StatesParser(ctx2);
    this.parseStyles = new StylesParser(ctx2);
  }
